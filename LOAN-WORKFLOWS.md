...
## Audit trail
Admins can open Audit trail to search by actor, action or record ID, filter UTC dates, and inspect before/after changes. Results are paginated at 50 events. The endpoint rejects non-admins; events are never included in member payloads or browser storage.

Starting with migration 0006, successful member enrollment/edits, organization changes, applications, approval/rejection steps, repayment settings, payments, waivers, payroll imports, profile edits, announcements and document uploads are recorded atomically with their database writes. Events capture the authenticated actor ID and name, server timestamp, target and selected before/after fields. Documents and message contents are not copied into the audit log. Payroll batches have a single event with deduction details. Failed/conflicting changes, previews and duplicate retries do not create success events. Database triggers reject event updates/deletes. This is application history, not a tamper-proof external compliance archive; database owners can alter schema. Earlier history is not backfilled. Demo history is in memory and resets on reload.
